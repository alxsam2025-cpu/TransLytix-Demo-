  ...
  const filtered = useMemo(() => {
    return stream
      .filter((r) => {
        ...
        if (
          query &&
          !(`${r.country} ${r.region} ${r.comment}`.toLowerCase().includes(query.toLowerCase()))
        )
          return false;
        return true;
      })
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6 p-4">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2">
        <h2 className="text-xl md:text-2xl font-bold">📑 Road Damage Reports</h2>
        <div className="text-sm text-gray-600">
          Total: <b>{totals.total}</b> — Critical:{" "}
          <b className="text-red-600">{totals.Critical}</b>
        </div>
      </div>
      ...
      <div className="bg-white rounded shadow p-4">
        <div className="flex flex-wrap gap-2">
          {SEVERITIES.map((s) => (
            <button
              ...
              className={`px-3 py-1 rounded text-sm ${
                filter === s ? "bg-green-600 text-white" : "bg-gray-200"
              }`}
            >
              {s}
            </button>
          ...
          <input
            placeholder="Search country/region/comment"
            value={query}
            onChange={(e) => setQuery(e.target.value)}
            className="border px-3 py-1 rounded text-sm flex-1 min-w-[200px]"
          />
        </div>
      </div>
              ...
              <div className="flex-1">
                ...
                <div className="mt-2 text-xs text-gray-600 flex flex-wrap gap-2">
                  ...
                  <div className="bg-gray-50 rounded p-1">
                    Unresolved: {r.unresolvedHours ?? 0} hrs
                  </div>
                </div>
              </div>
            </div>
